...
onspawn()
{
	...
	while(1)
	{
		...
		self thread bot_block_jugg_permaperk();
	}
}
...
bot_block_jugg_permaperk()
{
	...
	jugg_permaperk_stat = strtok("pers_jugg, pers_jugg_downgrade_count", ",");
	
	for(;;)
	{
		for(i = 0; i < jugg_permaperk_stat.size; i++)
			self maps\mp\zombies\_zm_stats::zero_client_stat(jugg_permaperk_stat[i], 0);
    ...
    if(!isdefined(self.bot.perk_purchase_time) || gettime() > self.bot.perk_purchase_time)
    {
		...
		if(level.round_number >= 8)
		{
			if(level.perk_purchase_limit == 4)
			{
				...
				{
					perks = array("specialty_quickrevive", "specialty_fastreload", "specialty_rof", "specialty_longersprint", "specialty_movefaster", "specialty_nomotionsensor", "specialty_additionalprimaryweapon");
					
					costs = array(1500, 3000, 2000, 2000, 2500, 3000, 4000);
				}
