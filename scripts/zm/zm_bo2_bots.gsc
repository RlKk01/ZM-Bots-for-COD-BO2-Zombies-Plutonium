...
bot_buy_perks()
{
    ...
    if (!isDefined(self.bot.perk_purchase_time) || GetTime() > self.bot.perk_purchase_time)
    {
		...
		{
			perks = array("specialty_fastreload", "specialty_rof", "specialty_longersprint", "specialty_nomotionsensor", "specialty_flakjacket", "specialty_grenadepulldeath");
			costs = array(3000, 2000, 2000, 3000, 2000, 2000);
		}
		else
		{
...
bot_buy_box()
{
    ...
    if (!isDefined(self.bot.box_purchase_time) || GetTime() > self.bot.box_purchase_time)
    {
        ...
        if(is_true(current_box._box_open) ||
           ...
           (isDefined(level.mystery_box_teddy_locations) && fast_array_contains(level.mystery_box_teddy_locations, current_box.origin))) // Avoid teddy locations
        {
...
bot_buy_wallbuy()
{
	...
	if (isSubStr(weapon, "staff") || isSubStr(weapon, "blunder") || isSubStr(weapon, "slowgun") || 
		...
		isSubStr(weapon, "tar21") || isSubStr(weapon, "type95") || isSubStr(weapon, "galil") || 
		isSubStr(weapon, "m16") || isSubStr(weapon, "ak47") || isSubStr(weapon, "mp44") || 
		
		isSubStr(weapon, "evoskorpion") || isSubStr(weapon, "pdw57") || 
		isSubStr(weapon, "mp5k") || isSubStr(weapon, "thompson") || 
		
		isSubStr(weapon, "dsr50") || isSubStr(weapon, "svu") || isSubStr(weapon, "barrett") || 
	...
	foreach(wallbuy in wallbuys)
	{
		...
		{
			if(bot_best_gun(wallbuy.trigger_stub.zombie_weapon_upgrade, weapon) && weapon != wallbuy.trigger_stub.zombie_weapon_upgrade && !is_offhand_weapon(wallbuy.trigger_stub.zombie_weapon_upgrade))
			{
				if(weapon == upgrade_name)
					return;
	...
	self AddGoal(weaponToBuy.origin, 9000000, 2, "weaponBuy");
